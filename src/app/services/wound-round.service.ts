...
export interface MobileRoundPatient {
  ...
  unit?: string | null;
  status: MobileRoundPatientStatus;
  assessmentIds?: string[];
  woundIds?: string[];
  evaluationStartedAt?: any;
  evaluatedAt?: any;
  visitedAt?: any;
  qaStatus?: 'not_ready' | 'ready' | 'approved' | 'returned';
  qaNote?: string | null;
  note?: string | null;
}
...
export interface MobileWoundRound {
  ...
  facilityName: string;
  roundDate: string;
  status: MobileRoundStatus;
  patients: MobileRoundPatient[];
  summary?: string | null;
  qaStatus?: 'not_ready' | 'ready' | 'approved' | 'returned';
  startedAt?: any;
  scheduledAt?: any;
...
export class WoundRoundMobileService {
  ...
  facilities$(): Observable<MobileFacility[]> {
    return new Observable(subscriber => {
      ...
      this.tenant.currentOrgId().then(orgId => {
        ...
        const q = query(collection(db, 'facilities'), where('orgId', '==', orgId));
        stop = onSnapshot(q, snap => {
          const rows = snap.docs
            .map(d => ({ id: d.id, ...d.data() } as MobileFacility))
  ...
  rounds$(): Observable<MobileWoundRound[]> {
    return new Observable(subscriber => {
      ...
      this.tenant.currentOrgId().then(orgId => {
        if (!orgId) { subscriber.next([]); return; }
        const q = query(collection(db, 'woundRounds'), where('orgId', '==', orgId));
        stop = onSnapshot(q, snap => {
          const rows = snap.docs
            .map(d => ({ id: d.id, ...d.data() } as MobileWoundRound))
            .sort((a, b) => this.roundSort(b) - this.roundSort(a));
          subscriber.next(rows);
        }, err => subscriber.error(err));
      }).catch(err => subscriber.error(err));
      return () => stop();
  ...
  round$(roundId: string): Observable<MobileWoundRound | null> {
    return new Observable(subscriber => {
      const stop = onSnapshot(doc(db, 'woundRounds', roundId), snap => {
        subscriber.next(snap.exists() ? ({ id: snap.id, ...snap.data() } as MobileWoundRound) : null);
      }, err => subscriber.error(err));
      return () => stop();
    });
  }
  ...
  async canAuthor(): Promise<boolean> {
    ...
    const roles = [token.claims['role'], ...(Array.isArray(token.claims['roles']) ? token.claims['roles'] as unknown[] : [])]
  ...
  async startRound(facility: MobileFacility, roundDate: string): Promise<string> {
    ...
    const patientQ = query(collection(db, 'patients'), where('orgId', '==', orgId));
    const patientSnap = await getDocs(patientQ);
    const patients: MobileRoundPatient[] = patientSnap.docs
      .map(d => ({ id: d.id, ...d.data() } as any))
      .filter(p => p.facilityId === facility.id && (!p.patientStatus || p.patientStatus === 'active'))
      .map(p => ({
        patientId: p.id,
        patientName: p.preferredName || p.name || 'Patient',
        ...
        evaluatedAt: null,
        visitedAt: null,
        qaStatus: 'not_ready' as const,
        note: null,
      }))
      .sort((a, b) => (a.roomNumber || '').localeCompare(b.roomNumber || '') || a.patientName.localeCompare(b.patientName));

    const token = await user.getIdTokenResult();
    const actor = {
      uid: user.uid,
      displayName: user.displayName || user.email || 'Clinician',
      role: String(token.claims['role'] || 'clinician'),
    };
  ...
  async startScheduled(roundId: string): Promise<void> {
    ...
    const facilitySnap = await getDoc(doc(db, 'facilities', round.facilityId));
    ...
    const patients: MobileRoundPatient[] = patientSnap.docs
      .map(d => ({ id: d.id, ...d.data() } as any))
      .filter(p => p.facilityId === round.facilityId && (!p.patientStatus || p.patientStatus === 'active'))
      .map(p => ({ patientId:p.id, patientName:p.preferredName || p.name || 'Patient', roomNumber:p.roomNumber ?? null, unit:p.unit ?? null, status:'pending', assessmentIds:[], woundIds:[], evaluationStartedAt:null, evaluatedAt:null, visitedAt:null, qaStatus:'not_ready', note:null }))
      .sort((a, b) => (a.roomNumber || '').localeCompare(b.roomNumber || '') || a.patientName.localeCompare(b.patientName));
    const token = await user.getIdTokenResult();
    await runTransaction(db, async tx => {
  ...
  async markPatient(roundId: string, patientId: string, status: MobileRoundPatientStatus, note?: string): Promise<void> {
    ...
    await runTransaction(db, async tx => {
      ...
      const patients = (round.patients || []).map((entry: MobileRoundPatient) => {
        ...
        found = true;
        if ((status === 'evaluated' || status === 'seen') && !(entry.assessmentIds || []).length) {
          throw new Error('At least one wound assessment must be linked before evaluation is complete');
        }
        return {
          ...entry,
          status,
  ...
  async completeRound(roundId: string): Promise<void> {
    ...
    await runTransaction(db, async tx => {
      ...
      if (!snap.exists()) throw new Error('Round not found');
      const round = snap.data() as any;
      const ready = (round.patients || []).every((p: MobileRoundPatient) => ['evaluated','seen','skipped'].includes(p.status));
      if (!ready) throw new Error('Resolve every patient before completing the round');
      tx.update(ref, {
        status:'completed',
        qaStatus:'ready',
        completedAt:serverTimestamp(),
        completedBy:{ uid:user.uid, displayName:user.displayName || user.email || 'Clinician', role:String(token.claims['role'] || 'clinician') },
        updatedAt:serverTimestamp(),
      });
    });
  }

  private roundSort(round: MobileWoundRound): number {
    const value = round.startedAt || round.scheduledAt || round.createdAt;
    if (value && typeof value.toMillis === 'function') return value.toMillis();
    const date = Date.parse(round.roundDate || '');
