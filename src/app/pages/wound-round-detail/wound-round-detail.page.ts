        ...
        <div class="workspace">
          ...
          <main class="patient-work" *ngIf="selected as p; else chooseTpl">
            ...
            <section class="action-grid" *ngIf="canAuthor && r.status==='in_progress'">
              ...
              <ion-button color="success" [disabled]="busy || p.status==='pending' || p.status==='evaluated' || p.status==='seen' || p.status==='skipped'" (click)="evaluate(p)"><ion-icon slot="start" [icon]="checkmarkCircleOutline"></ion-icon>Complete evaluation</ion-button>
            </section>
            <div class="workflow-note" *ngIf="p.status==='in_progress' && !(p.assessmentIds?.length)">
              <strong>Assessment in progress</strong>
              <span>Save the wound assessment to link it here. If the link is delayed, Complete evaluation will reconcile the latest assessment started during this round.</span>
            </div>
...
export class WoundRoundDetailPage implements OnInit,OnDestroy{
  ...
  async newAssessment(p:MobileRoundPatient){if(this.busy)return;this.busy=true;try{if(p.status==='pending')await this.rounds.markPatient(this.roundId,p.patientId,'in_progress');const opened=await this.router.navigate(['/tabs/skin-wound',p.patientId,'assessments/new'],{queryParams:{roundId:this.roundId}});if(!opened)throw new Error('Assessment screen did not open');}catch(e:any){this.busy=false;await this.message(e?.message||'Could not start patient assessment','danger')}}
  async openChart(p:MobileRoundPatient){if(this.busy)return;const opened=await this.router.navigate(['/tabs/skin-wound',p.patientId,'assessments'],{queryParams:{roundId:this.roundId}});if(!opened)await this.message('Could not open patient chart','danger')}
