...
import { IonBadge, IonButton, IonContent, IonHeader, IonIcon, IonInput, IonItem, IonNote, IonSpinner, IonTitle, IonToolbar, ToastController } from '@ionic/angular/standalone';
import { arrowBackOutline, checkmarkCircleOutline, clipboardOutline, documentTextOutline, personOutline, shieldCheckmarkOutline, sparklesOutline } from 'ionicons/icons';
...
@Component({
  ...
  standalone:true,
  imports:[CommonModule,FormsModule,IonHeader,IonToolbar,IonTitle,IonContent,IonButton,IonIcon,IonBadge,IonItem,IonInput,IonSpinner,IonNote],
  template:`
        ...
        <div class="workspace">
          ...
          <main class="patient-work" *ngIf="selected as p; else chooseTpl">
            ...
            <section class="clinical-card">
              <div class="clinical-stat"><span>Assessments linked</span><strong>{{ p.assessmentIds?.length || 0 }}</strong></div><div class="clinical-stat"><span>Wounds linked</span><strong>{{ p.woundIds?.length || 0 }}</strong></div><div class="clinical-stat"><span>QA state</span><strong>{{ qaLabel(p) }}</strong><small *ngIf="p.qaNote">{{ p.qaNote }}</small></div>
            </section>
            <section class="action-grid" *ngIf="canAuthor && r.status==='in_progress'">
              <ion-button fill="outline" [disabled]="busy" (click)="openChart(p)"><ion-icon slot="start" [icon]="documentTextOutline"></ion-icon>Open chart</ion-button>
              <ion-button [disabled]="busy" (click)="newAssessment(p)"><ion-icon slot="start" [icon]="clipboardOutline"></ion-icon>{{ (p.assessmentIds?.length || 0) ? 'Add wound assessment' : 'Start wound assessment' }}</ion-button>
              <ion-button color="success" [disabled]="busy || !(p.assessmentIds?.length) || p.status==='evaluated' || p.status==='seen' || p.status==='skipped'" (click)="evaluate(p)"><ion-icon slot="start" [icon]="checkmarkCircleOutline"></ion-icon>Complete evaluation</ion-button>
            </section>
            <div class="workflow-note" *ngIf="p.status==='in_progress' && !(p.assessmentIds?.length)">
              <strong>Assessment required</strong>
              <span>Open and save a wound assessment. The saved assessment will link back to this patient in the active round before evaluation can be completed.</span>
            </div>
            <div class="workflow-note linked" *ngIf="p.status==='in_progress' && (p.assessmentIds?.length)">
              <strong>Assessment linked</strong>
              <span>{{ p.assessmentIds?.length }} assessment(s) and {{ p.woundIds?.length || 0 }} wound(s) are attached to this round. Complete evaluation when documentation is ready for QA.</span>
            </div>
          ...
          <aside class="summary">
            ...
            <div class="qa"><ion-icon [icon]="sparklesOutline"></ion-icon><div><strong>QA readiness</strong><p>{{ resolved===r.patients.length ? 'Round can be closed and sent to QA.' : 'Each patient needs a linked assessment or a documented skip reason.' }}</p></div></div>
...
export class WoundRoundDetailPage implements OnInit,OnDestroy{
  ...
  async ngOnInit(){this.roundId=this.route.snapshot.paramMap.get('roundId')||'';this.focusPatientId=this.route.snapshot.queryParamMap.get('patientId')||'';this.canAuthor=await this.rounds.canAuthor().catch(()=>false);this.sub=this.rounds.round$(this.roundId).subscribe({next:r=>{this.round=r;if(r&&!this.selected){this.selected=(this.focusPatientId?r.patients.find(p=>p.patientId===this.focusPatientId):null)||r.patients.find(p=>p.status==='in_progress')||r.patients.find(p=>p.status==='pending')||r.patients[0]||null;}else if(r&&this.selected)this.selected=r.patients.find(p=>p.patientId===this.selected?.patientId)||null},error:async()=>this.message('Unable to open round','danger')});}
  ...
  qaLabel(p:MobileRoundPatient){if(p.qaStatus==='approved')return'approved';if(p.qaStatus==='returned')return'returned';if(p.status==='evaluated'||p.status==='seen')return'ready';if((p.assessmentIds?.length||0)>0)return'assessment_linked';return'not_ready'}
  async newAssessment(p:MobileRoundPatient){if(this.busy)return;this.busy=true;const wasPending=p.status==='pending';try{if(wasPending)await this.rounds.markPatient(this.roundId,p.patientId,'in_progress');const opened=await this.router.navigate(['/tabs','skin-wound',p.patientId,'assessments','new'],{queryParams:{roundId:this.roundId,roundPatientId:p.patientId}});if(!opened)throw new Error('Assessment screen did not open');}catch(e:any){if(wasPending){await this.rounds.markPatient(this.roundId,p.patientId,'pending').catch(()=>undefined)}this.busy=false;await this.message(e?.message||'Could not start patient assessment','danger')}}
  async openChart(p:MobileRoundPatient){if(this.busy)return;const opened=await this.router.navigate(['/tabs','skin-wound',p.patientId,'assessments'],{queryParams:{roundId:this.roundId,roundPatientId:p.patientId}});if(!opened)await this.message('Could not open patient chart','danger')}
  async evaluate(p:MobileRoundPatient){this.busy=true;try{await this.rounds.markPatient(this.roundId,p.patientId,'evaluated');await this.message('Patient evaluation completed and ready for QA');this.selectNext()}catch(e:any){await this.message(e?.message||'Assessment required before completion','danger')}finally{this.busy=false}}
