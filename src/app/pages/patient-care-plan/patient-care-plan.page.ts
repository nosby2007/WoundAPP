...
export class PatientCarePlanPage implements OnInit{
  ...
  async save(){if(this.form.invalid||this.saving)return;const customGoals=splitLines(this.form.value.customGoals);const refs=[...this.selectedGoalIds];if(!customGoals.length&&!refs.length){this.errorMsg='Pick at least one organization goal or write a custom goal.';return;}this.saving=true;this.errorMsg='';try{await this.carePlans.create(this.patientId,{title:this.form.value.title!,description:this.form.value.description||null,startDate:this.form.value.startDate!,woundId:this.woundId||null,category:this.form.value.category as CarePlanProblemCategory,goalCatalogRefs:refs,customGoals},{visitId:this.woundVisitId||null,appointmentId:this.appointmentId||null,woundId:this.woundId||null,episodeId:this.episodeId||null,fieldEncounterVisitId:this.woundVisitId||null});const t=await this.toast.create({message:'Care plan saved',duration:2200,color:'success'});await t.present();await this.router.navigate(['/tabs','skin-wound',this.patientId,'assessments'],{queryParams:clinicalVisitQueryParams({visitId:this.woundVisitId||null,appointmentId:this.appointmentId||null,woundId:this.woundId||null,episodeId:this.episodeId||null})});}catch(e:any){this.errorMsg=e?.message||'Could not save care plan.';}finally{this.saving=false;}}
}
